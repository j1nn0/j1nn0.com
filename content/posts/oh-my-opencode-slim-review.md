...
| エージェント | モデル | 主な用途 |
|---|---|---|
| Orchestrator | opencode-go/deepseek-v4-pro | 計画、委譲、整合、検証 |
...
重い作業は安いモデル、精度が要る場面だけ高性能モデル、という割り振りにしている。

Oracle に GPT-5.5 を割り当てているのは、アーキテクチャの判断や難しいデバッグには性能のいいモデルを使うべきだと考えたからだ。
精度が求められる場面に絞って使うことで、ChatGPT Plus のサブスクを有効活用できる。

OpenCode Go と Ollama Cloud は利用できるモデルが重なっている部分があるため、どちらに何を割り当てるかはコストのバランスで決めた。
